...
export function LocationCard({ location, onCheckIn }: LocationCardProps) {
  ...
  return (
    <button
      type="button"
      onClick={handleClick}
      className={`group relative min-h-[148px] overflow-hidden rounded-2xl border p-6 text-left backdrop-blur-2xl transition-all duration-300 ${
      ...
      <div className="relative z-10 flex h-full flex-col justify-between gap-4">
        ...
        <div>
          ...
          {location.description ? (
            ...
            <p className="mt-1 text-sm text-white/40">Tap to check in</p>
          )}
        </div>
      </div>
    </button>
